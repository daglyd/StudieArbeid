...
class ProblemSIRV(ProblemSIR):
    ...
    def __call__(self,u,t):
        S, I, R, V = u
        rhs = -self.beta(t)*S*I - self.p*S, self.beta(t)*S*I - self.nu(t)*I, self.nu(t)*I,\
        self.p*S
        return rhs
...
problem = ProblemSIRV(beta=lambda t : 0.0005 if t <=12 else 0.0001, nu=0.1,
                     S0=1500, I0=1, R0=0, T=60, V0=0, p = 0.1)
solver = SolverSIRV(problem,0.5)
solver.solve()
solver.plot()

"""
The total number of infected is drastically reduced when vaccination is introduced.
"""
"""
In [84]: run  SIRV.py
